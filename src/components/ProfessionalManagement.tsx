import { useEffect, useState } from "react";
import app from "../firebaseConfig";
import { getDatabase, ref, onValue, remove, update } from "firebase/database";
import {
  Table,
  Container,
  Row,
  Col,
  Button,
  Form,
  Spinner,
} from "react-bootstrap";

const database = getDatabase(app);

interface Professional {
  profissao: string;
  registroConselho: string;
  personID: string;
}

interface Person {
  name: string;
  surname: string;
  cpf: string;
  rg: string;
  email: string;
  contact: string;
  addressId: string;
}

function ProfessionalManagement() {
  const [professional, setProfessional] = useState<
    Record<string, Professional>
  >({});
  const [person, setPerson] = useState<Record<string, Person>>({});
  const [editId, setEditId] = useState<string | null>(null);
  const [editData, setEditData] = useState<Partial<Professional>>({});
  const [loading, setLoading] = useState<boolean>(true);
  const [loading2, setLoading2] = useState<boolean>(true);

  useEffect(() => {
    // Fetch all professionals
    const professionalRef = ref(database, "professional");
    const unsubscribeProfessional = onValue(professionalRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        setProfessional(data);
        setLoading2(false);
      }
    });

    // Fetch all persons
    const personRef = ref(database, "persons");
    const unsubscribePerson = onValue(personRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        setPerson(data);
        setLoading(false);
      }
    });

    return () => {
      unsubscribeProfessional();
      unsubscribePerson();
    };
  }, []);

  const handleEdit = (id: string) => {
    const professionalToEdit = professional[id];
    setEditId(id);
    setEditData(professionalToEdit);
  };

  const handleRemove = (id: string) => {
    const professionalRef = ref(database, `professional/${id}`);
    remove(professionalRef).then(() => {
      const newProfessional = { ...professional };
      delete newProfessional[id];
      setProfessional(newProfessional);
    });
  };

  const handleSave = () => {
    if (editId && editData) {
      const professionalRef = ref(database, `professional/${editId}`);
      update(professionalRef, editData).then(() => {
        setProfessional({
          ...professional,
          [editId]: { ...professional[editId], ...editData },
        });
        setEditId(null);
        setEditData({});
      });
    }
  };

  if (loading || loading2) {
    return (
      <Container fluid className="text-center">
        <Spinner animation="border" variant="primary" />
        <p>Loading...</p>
      </Container>
    );
  }

  return (
    <>
      <h3 className="centered-header">Professionals</h3>
      <Container fluid>
        <Row className="justify-content-md-center">
          <Col xs={12} md={11}>
            <Table bordered>
              <thead>
                <tr>
                  <th>Name</th>
                  <th>Surname</th>
                  <th>CPF</th>
                  <th>RG</th>
                  <th>Email</th>
                  <th>Contact</th>
                  <th>Profession</th>
                  <th>Board Registration</th>
                  <th colSpan={2}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(professional).map(([key, professional]) => {
                  const persons = person[professional.personID]; // Find the person by personID

                  return (
                    <tr key={key}>
                      <td>{persons ? persons.name : "N/A"}</td>
                      <td>{persons ? persons.surname : "N/A"}</td>
                      <td>{persons ? persons.cpf : "N/A"}</td>
                      <td>{persons ? persons.rg : "N/A"}</td>
                      <td>{persons ? persons.email : "N/A"}</td>
                      <td>{persons ? persons.contact : "N/A"}</td>
                      <td>{professional.profissao}</td>
                      <td>{professional.registroConselho}</td>
                      <td>
                        <Button
                          variant="warning"
                          onClick={() => handleEdit(key)}
                          className="w-100"
                        >
                          Edit
                        </Button>
                      </td>
                      <td>
                        <Button
                          variant="danger"
                          onClick={() => handleRemove(key)}
                          className="w-100"
                        >
                          Remove
                        </Button>
                      </td>
                    </tr>
                  );
                })}
              </tbody>
            </Table>
          </Col>
        </Row>
        {editId && (
          <Row className="justify-content-md-center">
            <Col xs={12} md={6} className="bordered-form">
              <p className="initial" />
              <h5>Edit Professional</h5>
              <Form>
                <Form.Group className="mb-1">
                  <Form.Label>Profession</Form.Label>
                  <Form.Control
                    type="text"
                    value={editData.profissao || ""}
                    onChange={(e) =>
                      setEditData({ ...editData, profissao: e.target.value })
                    }
                    className="w-100"
                  />
                </Form.Group>
                <Form.Group className="mb-1">
                  <Form.Label>Board Registration</Form.Label>
                  <Form.Control
                    type="text"
                    value={editData.registroConselho || ""}
                    onChange={(e) =>
                      setEditData({
                        ...editData,
                        registroConselho: e.target.value,
                      })
                    }
                    className="w-100"
                  />
                </Form.Group>
                <Form.Group className="mb-1">
                  <Form.Label>Select Person</Form.Label>
                  <Form.Control
                    as="select"
                    value={editData.personID || ""}
                    onChange={(e) =>
                      setEditData({ ...editData, personID: e.target.value })
                    }
                    className="w-100"
                  >
                    <option value="">Select a person</option>
                    {Object.entries(person).map(([key, person]) => (
                      <option key={key} value={key}>
                        {`${person.name} ${person.surname}`}
                      </option>
                    ))}
                  </Form.Control>
                </Form.Group>
                <p className="initial" />
                <Button variant="success" onClick={handleSave} className="w-50">
                  Save Changes
                </Button>
                <p className="initial" />
              </Form>
            </Col>
          </Row>
        )}
      </Container>
      <p className="initial" />
    </>
  );
}

export default ProfessionalManagement;
